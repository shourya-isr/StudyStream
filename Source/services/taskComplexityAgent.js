...
// Use provided assistant ID and strict instructions
  ...
  const thread = await openai.beta.threads.create({
    messages: [
      {
        role: "user",
        content: question && question.trim().length > 0 ? question : "Analyze the attached assignment PDF and break it down into actionable steps for scheduling.",
...
export default {
  analyze: async function(pdfPath) {
    // For compatibility, call analyze with only pdfPath
