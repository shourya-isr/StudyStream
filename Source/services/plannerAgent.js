...
async function plan({ assignment, scrubbedTaskPlans, unavailableSlots, conflictingTasks, earliestStart, mode, feedback }) {
  // Choose instructions and prompt based on mode
  let instructions;
  let prompt;
  if (mode === 'replan') {
    instructions = instructionsReplan;
    prompt = `You are a replanning agent. Given the following assignment, active tasks, unavailable slots, conflicting tasks, the earliest possible start time, and telemetry feedback, update the schedule to minimize changes and resolve conflicts. Each session must be at least 30 minutes. No session can be scheduled before the earliestStart timestamp. Output strict JSON.\nAssignment: ${JSON.stringify(assignment)}\nActive Tasks: ${JSON.stringify(scrubbedTaskPlans)}\nUnavailable Slots: ${JSON.stringify(unavailableSlots)}\nConflicting Tasks: ${JSON.stringify(conflictingTasks)}\nEarliest Start: ${earliestStart}\nFeedback: ${feedback || ''}`;
  } else {
