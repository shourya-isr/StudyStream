...
async function plan({ assignment, scrubbedTaskPlans, unavailableSlots, conflictingTasks, earliestStart }) {
  // Build prompt for OpenAI agent
  const prompt = `You are a scheduling agent. Given the following assignment, task plans, unavailable slots, conflicting tasks, and the earliest possible start time, generate a schedule with no conflicts. Each session must be at least 30 minutes. No session can be scheduled before the earliestStart timestamp. Output strict JSON.\nAssignment: ${JSON.stringify(assignment)}\nTask Plans: ${JSON.stringify(scrubbedTaskPlans)}\nUnavailable Slots: ${JSON.stringify(unavailableSlots)}\nConflicting Tasks: ${JSON.stringify(conflictingTasks)}\nEarliest Start: ${earliestStart}`;
